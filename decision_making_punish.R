...
# create variable for % of National GDP held on offshore accounts by households. Data from 
...
for (g in 1:ngroups) {
  ...
  for (s in 1:groupSize) {
    singlechild[s, g] <- redDat$singlechild[redDat$groupid == group_names[g] & redDat$p == "P-experiment"][s]
...
# Redefine the number of groups after removing those without civic scores
ngroups <- length(Gini)

# Aggregate % of National GDP held on offshore accounts by households to just 1 number per Nation-index (using the mean here should be unproblematic since all groups within a given nation should have been given the same Gini-coefficient)
Gini <- aggregate(Gini ~ Nation, FUN = mean)[, 2]

nnations <- length(Gini)

# Calculate the winnings (i.e. apply the multiplication-factor to the sum of each group's contributions)
...
#-------------------  Regress % of National GDP held in tax havens by households on winnings ---------------
...
#-------------------  Regress % of National GDP held in tax havens by households on belief weights and slope of prefs in CC model ---------------
...
png(filename="PGG_offshore_tax_figure_4_punishment.png", height = 800, width = 1000)
layout(matrix(1:2, nrow = 1))

# Graph G: Data - Contribution
par(mar=c(5,4,4,2) + 0.1)
plot(c(1,65), c(0,1300), type = "n", main = "G: Group Contribution - punisment condition", 
...
# Graph H: Posterior of effect - % of National GDP held on offshore accounts by households and group winnings with adjusted 95% interval
par(mar=c(5,4,4,2) + 0.1)
plot(density(win.samples$BUGSoutput$sims.list$betaX), frame=FALSE, lwd=2, ylim=c(0,7),
     main="H: Mean Posterior Contribution - punishment condition", xlab = "Standardised % of National GDP held in tax havens by households", ylab = "")
...
png(filename="PGG_offshore_tax_figure_5_punishment.png", height = 800, width = 1000)
layout(matrix(1:2, nrow = 1))

# Graph J: Data - Initial Contribution
par(mar=c(5,4,4,2) + 0.1)
plot(c(1,65), c(0,30), type = "n", main = "J: Initial Contribution - punishment condition", 
...
# Graph K: Initial Belief with adjusted 95% interval
par(mar=c(5,4,4,2) + 0.1)
plot(density(CC.samples$BUGSoutput$sims.list$betaX_alpha), frame=FALSE, lwd=2, ylim=c(0,15),
     main="K: Initial Belief - punishment condition", xlab = "Standardised % of National GDP held in tax havens by households", ylab = "")
abline(v=0, lty=2) # Zero line for reference
...
png(filename="PGG_offshore_tax_figure_6_punishment.png", height = 800, width = 1000)
layout(matrix(1:2, nrow = 1))

# Graph L: Belief Learning Weight with adjusted 95% interval
par(mar=c(5,4,4,2) + 0.1)
plot(density(CC.samples$BUGSoutput$sims.list$betaX_omega), frame=FALSE, lwd=2, ylim=c(0,10),
     main="L: Belief Learning Weight - punishment condition", xlab = "Standardised % of National GDP held in tax havens by households", ylab = "")
abline(v=0, lty=2) # Zero line for reference
y_pos_e <- -0.05
...
# Graph M: Conditional Preferences with adjusted 95% interval
par(mar=c(5,4,4,2) + 0.1)
plot(density(CC.samples$BUGSoutput$sims.list$betaX_rho), frame=FALSE, lwd=2, ylim=c(0,10),
     main="M: Conditional Preferences - punishment condition", xlab = "Standardised % of National GDP held in tax havens by households", ylab = "")
abline(v=0, lty=2) # Zero line for reference
y_pos_f <- -0.05
# Drawing the 95% Bayesian Credible Interval at the bottom
